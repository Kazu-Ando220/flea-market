...
① 未ログインユーザーへのログイン誘導モーダル
未ログイン状態（ゲスト）で、いいね・コメント送信・購入手続きなどログインが必要な操作を行った際に、ログイン画面・会員登録画面へのリンクを含むモーダルを表示し、離脱を防ぐ導線を設けた。

※ `test2@example.com` は「未ログイン」ではなく「ログイン済み・メール未認証ユーザー」です。このため本モーダルは表示されず、認証が必要な操作ではメール認証画面への導線が適用されます。
